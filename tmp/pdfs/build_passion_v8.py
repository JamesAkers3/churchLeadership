...
OUTPUT = ROOT / "output" / "pdf" / "from-passion-to-impact-v8.pdf"
...
def focused_journey(c):
    ...
    stage(c, 55, 446, 78, "PASSION", "the energy we carry")
    stage(c, 146, 446, 102, "VISION", "a future picture that focuses us")
    stage(c, 276, 446, 120, "MISSION", "faithful action that directs us")
    stage(c, 424, 446, 138, "IMPACT", "what happens where that action reaches people", body_size=7.8)


def limited_journey(c):
    c.setFillColor(optics.PALE_RUST)
    c.roundRect(42, 181, 528, 212, 14, fill=1, stroke=0)
    panel_header(c, "LIMITED IMPACT", "(PASSION - CLEAR VISION) / COMPETING MISSIONS = LIMITED IMPACT", 366, optics.RUST)
    ...
    endpoints = [(372, 347), (450, 321), (478, 242)]
    ...
    stage(c, 143, 216, 110, "UNCLEAR VISION", "blurs the picture and spreads the energy", optics.RUST, body_size=7.9)
    stage(c, 276, 216, 120, "COMPETING MISSIONS", "pull the work in different directions", optics.RUST, body_size=7.9)
    stage(c, 431, 216, 124, "LIMITED IMPACT", "less of the work reaches the people it was meant to serve", optics.RUST, body_size=7.8)


def build():
    optics.register_fonts()
    c = canvas.Canvas(str(OUTPUT), pagesize=LETTER, pageCompression=1, initialFontName="Arial")
    c.setTitle("From Passion to Impact")
    c.setSubject("How a faithful vision and mission help direct passion toward life-giving impact")
    ...
    optics.paragraph(c, "But without vision to focus it and mission to direct it, passion becomes scattered energy.", 43, 682, 520, opening)
    ...
    c.drawString(58, 108, "Passion is the energy. Vision brings it into focus.")
    c.drawString(58, 88, "Mission points it somewhere that matters. Impact happens where it lands.")
    c.setFillColor(HexColor("#D8E1DB"))
    c.setFont("Arial", 7.8)
    c.drawString(58, 70, "Jesus knew why He came and stayed faithful to the Father's purpose. We let Jesus direct what we carry and where we go.")
