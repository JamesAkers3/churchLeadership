...
SUPPORT_OUTPUT = ROOT / "output/pdf/a-pastor-can-be-known-and-still-unsupported-v8.pdf"
...
def stat_column(pdf, x, top, width, label, value, copy, value_color):
    ...
    draw_paragraph(pdf, copy, copy_style, x + 18, top - 52, width - 36)
...
def build_support_pdf() -> None:
    ...
    stat_column(
        ...
        "LONELY OR ISOLATED",
        "65%",
        "report feeling lonely or isolated at least sometimes.",
        HexColor("#8A5C00"),
    )
    ...
    bridge_body = style("bridge-body", "Helvetica", 10, 13, INK, TA_CENTER)
    draw_paragraph(
        pdf,
    ...
    pdf.roundRect(margin, 153, content_width, 126, 12, fill=1, stroke=0)
    pdf.setStrokeColor(GOLD)
    pdf.setLineWidth(3)
    pdf.line(margin + 18, 165, margin + 18, 263)
    question = style("support-question", "Georgia-Bold", 22, 26, FOREST)
    question_body = style("support-question-body", "Helvetica", 10.5, 14, INK)
    next_step = style("support-next-step", "Helvetica", 10.5, 14, FOREST)
    draw_paragraph(
        pdf,
    ...
    draw_paragraph(
        pdf,
        "<b>Begin with one safe, honest conversation.</b><br/>You do not have to carry this alone or wait for a crisis to ask for support.",
        next_step,
        margin + 34,
        194,
        content_width - 58,
